...
How often a write **reads** an existing record depends on **`recordExistsAction`** when **sendKey** is off—see [policy-replace-whole-record.md](aerospike-development-policy-replace-whole-record.md). For example, **`REPLACE`** / **`REPLACE_ONLY`** can skip that read. With **sendKey** on, the server still **reads** the record when one exists so it can **verify the sent user key against the digest** (and persist or update the stored key)—even when the exist action would otherwise avoid a merge read.
