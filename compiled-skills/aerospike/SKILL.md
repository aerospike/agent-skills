...
**Reading a rule in full.** Each rule below states its instruction and nothing more; the reasoning, the worked detail and the documentation links live in its own file, shipped in the `references/` folder beside this one. A rule's file is `references/<skill>-<rule>.md`, where `<skill>` is the `##` heading it sits under and `<rule>` is its own `###` heading — so `client-singleton` under `aerospike-development` is `references/aerospike-development-client-singleton.md`. Rules cite each other by bare filename and resolve the same way._
