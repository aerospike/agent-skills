...
def _header(skill_dirs: list[str]) -> str:
    return (
        ...
        f"\n"
        f"**Reading a rule in full.** Rules are grouped under a `###` heading naming "
        f"their filename prefix (`client-`, `policy-`, …), explained in the "
        f"skill's own prefix table. Each rule is a `####` heading of the form "
        f"`<rule> — <title> [IMPACT]`. Each states its "
